...
for distance_idx, distance in enumerate(distances):
    ...
    for bit_length_idx, n in enumerate(bit_lengths):
        matching_bases = []
        matching_indices = []
        bob_sifted_key = []
        alice_sifted_key = []
        ...
        key_rates[distance_idx, bit_length_idx] = key_rate

        data_loss_rates1[distance_idx, bit_length_idx] = uncorrected_error_rate
        data_loss_rates2[distance_idx, bit_length_idx] = corrected_error_rate
        error_rates1[distance_idx, bit_length_idx] = uncorrected_error_rate
        error_rates2[distance_idx, bit_length_idx] = corrected_error_rate

tones_of_red = ['darkred', 'red', 'indianred', 'firebrick', 'crimson', 'tomato', 'salmon', 'coral', 'orangered', 'darkorange', 'saddlebrown']
tones_of_blue = ['darkblue', 'blue', 'mediumblue', 'royalblue', 'dodgerblue', 'deepskyblue', 'skyblue', 'lightblue', 'steelblue', 'cornflowerblue', 'darkslateblue']
...
for distance_idx, distance in enumerate(distances):
    alpha = 0.6 # Calculate alpha value for color
    plt.scatter(bit_lengths, data_loss_rates1[distance_idx], color=tones_of_blue[distance_idx], alpha=alpha, s=8)
    plt.scatter(bit_lengths, data_loss_rates2[distance_idx], color=tones_of_red[distance_idx], alpha=alpha, s=8)
...
for distance_idx, distance in enumerate(distances):
    ...
    legend_elements1.append(Line2D([0], [0], marker='o', c='w',markerfacecolor=tones_of_red[distance_idx], markersize=8,
                                  alpha=alpha,label=label))
    legend_elements2.append(Line2D([0], [0], marker='o',c='w',markerfacecolor=tones_of_blue[distance_idx], markersize=8,
                                  alpha=alpha, label=label))
...
for distance_idx, distance in enumerate(distances):
    alpha=0.6
    plt.plot(bit_lengths, error_rates1[distance_idx],'o',c=tones_of_blue[distance_idx],linewidth=1, markersize=3,alpha=alpha)
    plt.plot(bit_lengths, error_rates2[distance_idx],'o',c=tones_of_red[distance_idx],linewidth=1, markersize=3,alpha=alpha)
...
legend_elements2.append(Line2D([0], [0],color='orange', markersize=8,label='Uncorrected'))
legend_elements1.append(Line2D([0], [0],color='purple', markersize=8,label='Corrected'))
